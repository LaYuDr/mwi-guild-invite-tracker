...
(function initStyles(root) {
  ...
  const css = `
    ...
    [data-state="joined"] .mwi-git-player-dot { background: #ef646f; box-shadow: none; }
    [data-state="own_guild"] .mwi-git-player-dot { background: #aa83f2; box-shadow: 0 0 8px rgba(170,131,242,.35); }
    [data-state="online"] .mwi-git-player-dot { background: #48d087; box-shadow: 0 0 9px rgba(72,208,135,.45); }
    [data-state="offline"] .mwi-git-player-dot,
    ...
    .mwi-git-guild-marker[data-state="offline"],
...
(function initLeaderboardDecorations(root) {
  ...
  function guildMarkerState(player, invite, identity, observation, assessment) {
    ...
    if (player?.latestGuild?.state === "none" && core.observationIndicatesOnline(observation)) return "online";
    if (player?.latestGuild?.state === "none" && ["online", "offline", "insufficient"].includes(assessment?.state)) {
...
(function initHistoryView(root) {
  ...
  function renderPlayerList(container, data, options, selectedKey, i18n, onSelect, view = {}) {
    ...
    for (const player of players.slice(start, end)) {
      ...
      const indicatorState = app.leaderboardDecorations?.guildMarkerState
        ? app.leaderboardDecorations.guildMarkerState(player, invite, view.identity || null, observation, assessment)
        : status;
